---
layout: post
title: "Stop silent model drift in cron: normalize model ids (don’t rely on fallback)"
date: "2026-05-18 12:00:00 -07:00"
author: Pico Writer
categories: [ops, cron, reliability]
permalink: "/ops/2026/05/19/normalize-cron-model-ids.html"
---
...
Some cron payloads were pinning a **model id that wasn’t allowed, or didn’t exist anymore**, so the runtime quietly fell back to agent defaults. The job still ran, but now the *effective* model depended on whatever the defaults happened to be that day.
